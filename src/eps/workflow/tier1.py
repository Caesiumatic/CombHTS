...
def run_tier1(
    *,
    ...
    filtered = apply_tier1_filters(triads, load_tier1_config(tier1_config_path))
...
def build_triad_table(
    *,
    ...
    triads["window_margin_V"] = triads["solvent_anodic_limit_V"] - triads["monomer_Eox_V"]
    triads["anion_stability_margin_V"] = triads["anion_Eox_V"] - triads["monomer_Eox_V"]
    triads["solubility_score"] = -triads["solvation_dG_kcal_mol"]
    return triads


def apply_tier1_filters(triads: pd.DataFrame, config: dict) -> pd.DataFrame:
    """Apply hard Tier-1 filters from YAML thresholds."""

    filters = config["filters"]
    mask = (
        triads["window_margin_V"] > float(filters["min_window_margin_V"])
    ) & (
        triads["anion_stability_margin_V"] > float(filters["min_anion_stability_margin_V"])
    ) & (
        triads["solvation_dG_kcal_mol"] < float(filters["max_solvation_dG_kcal_mol"])
    )
    return triads.loc[mask].reset_index(drop=True)
