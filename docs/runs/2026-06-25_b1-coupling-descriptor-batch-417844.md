...
- cluster job: SGE 417844 → 417845 (both failed, see below) → **SGE 417846** (authoritative), `-pe smp 8`, h_rt 4:00:00
- status: running (417846, queued). Two prior failures, both caught loudly (no wrong-data run): 417844 qdel'd (compute-node `rsync` absent + no `set -e` → fell back to the installed eps reading the PRODUCTION library); 417845 failed at load (B1 monomers.csv had unquoted commas in the notes field — "2,5-alpha" etc. — breaking the CSV). Fixes: `cp -a` + `set -euo pipefail` + eps-path/monomer-count asserts (script), and quoted notes fields (CSVs). CSVs now validate locally via the real eps loaders; dimer-build preview confirms signal #1 (2,5-dimethylpyrrole fails the build = α,α′-blocked; the 9 free-α monomers build).
