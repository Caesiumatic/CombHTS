...
def test_default_real_benchmark_selection_excludes_aqueous_and_tier_c(tmp_path: Path) -> None:
    ...
    assert int(selected.sum()) == 49
    assert result.raw_benchmark_rows == 49
    assert result.calibration_eligible_rows == 49
    assert result.n_calibration_points == 48  # 49 rows; only the two pyrrole peak rows collapse to one group
...
def test_strict_benchmark_v1_collapses_by_smiles_solvent_and_label(tmp_path: Path) -> None:
    ...
    assert result.n_calibration_points == 48
...
def test_peak_and_onset_profiles_use_disjoint_groups_and_different_fits(tmp_path: Path) -> None:
    ...
    assert onset.n_calibration_points == 19
    assert (
