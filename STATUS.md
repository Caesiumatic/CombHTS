...
- `eps analyze` (directive §8, read-only) turns a Tier-1 harvest CSV into `summary.csv` (total/surviving/retention overall + by monomer/solvent/salt_class + per-property failure counts), real-axis distribution PNGs, a Pareto PNG, a Morgan-fingerprint+descriptor chemical-space map (PCA→t-SNE, PCA(2) fallback for n<10), and a DIAGNOSTIC-ONLY `shortlist.csv`. Any output touching `optical_gap`/`dimerization_dG`/`band_gap_deviation_eV`/`composite_score`/`pareto_front` is labeled placeholder-contaminated. Degrades gracefully if matplotlib/scikit-learn are absent or if the harvest lacks scoring columns (the all-triads audit currently does — point analyze at a scored CSV for the Pareto/shortlist). `matplotlib`+`scikit-learn` added as deps.
...
- The clean real-xTB harvest is a real-physics milestone, NOT a final scientific ranking. The composite RANKING is still diagnostic: `optical_gap` and `dimerization_dG` remain placeholders (T6), so two of the scoring axes are not yet real physics. The anion Eox is on a monomer-fit calibration extrapolation (T11), so the anion-stability margin is coarse. Zero calculation failures means every stage produced a value, not that every value is validated.
